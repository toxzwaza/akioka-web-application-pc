...
watch(() => props.gantt_update, (newVal) => {
  if (newVal) {
...
const getGantData = () => {
  axios.get(route('task.getGanttData'))
    .then(res => {
      if (isMounted.value) { // アンマウント済みなら何もしない
...
<template>
  <div style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;" class="overflow-x-auto">
    <table class="table-auto border-collapse w-full text-xs">
      <thead>
        <tr>
          <th class="bg-gray-50 border px-4 py-2 w-40 whitespace-nowrap">ユーザー名</th>
          <th class="bg-gray-50 border px-4 py-2 w-40 whitespace-nowrap">タスク名</th>
          <th
      ...
      <tbody>
        <template v-for="(user, userIndex) in tasks" :key="userIndex">
          <tr v-for="(task, taskIndex) in user.tasks" :key="taskIndex">
            ...
            <td class="bg-gray-50 border px-2  whitespace-nowrap">{{ task.name }}</td>
            <td
              v-for="slot in timeSlots"
              :key="slot.label"
            ...
            >
              <div
                :class="[isActive(slot.value, task.periods) ? task.color : '', 'h-full w-full']"
              />
            </td>
          </tr>
        </template>
