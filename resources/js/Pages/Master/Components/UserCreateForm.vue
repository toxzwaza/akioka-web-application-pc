<script setup>
import { reactive } from "vue";
import axios from "axios";

const props = defineProps({
  groups: Array,
  processes: Array,
  positions: Array,
});

const emit = defineEmits(["registered"]);

const form = reactive({
  emp_no: null,
  gender_flg: 0,
  name: null,
  email: null,
  password: null,
  group_id: 0,
  process_id: 0,
  position_id: 0,
  fax_folder_name: null,
  is_admin: null,
  dispatch_flg: null,
  part_flg: null,
  always_order_flg: null,
});

const createUser = () => {
  axios
    .post(route("master.store.users"), form)
    .then((res) => {
      if (res.data.status) {
        if (confirm("ユーザー登録が完了しました。続けて登録を行いますか？")) {
          Object.assign(form, {
            emp_no: null,
            gender_flg: 0,
            name: null,
            email: null,
            password: null,
            group_id: 0,
            process_id: 0,
            position_id: 0,
            fax_folder_name: null,
            is_admin: null,
            dispatch_flg: null,
            part_flg: null,
            always_order_flg: null,
          });
        }
        emit("registered");
      }
    })
    .catch((error) => {
      console.log(error);
    });
};
</script>
<template>
  <form
    method="post"
    class="mx-auto grid max-w-screen-md gap-4 sm:grid-cols-2"
    @submit.prevent
  >
    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >社員番号</label
      >
      <input
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.emp_no"
      />
    </div>
    <div class="sm:col-span-2">
      <label
        :class="{
          'mb-2 inline-block text-sm text-gray-800 sm:text-base': true,
          'text-red-500': !form.name,
        }"
        >氏名</label
      >
      <input
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.name"
      />
    </div>
    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >Email</label
      >
      <input
        type="email"
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.email"
      />
    </div>

    <div class="sm:col-span-2">
      <label
        :class="{
          'mb-2 inline-block text-sm text-gray-800 sm:text-base': true,
          'text-red-500': !form.password,
        }"
        >パスワード</label
      >
      <input
        type="password"
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.password"
      />
    </div>

    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >性別</label
      >
      <div class="flex items-center">
        <label class="mr-4">
          <input type="radio" name="gender_flg" :value="0" v-model="form.gender_flg" />
          男性
        </label>
        <label>
          <input type="radio" name="gender_flg" :value="1" v-model="form.gender_flg" />
          女性
        </label>
      </div>
    </div>

    <hr class="my-8 sm:col-span-2" />

    <div class="sm:col-span-2">
      <label
        :class="{
          'mb-2 inline-block text-sm text-gray-800 sm:text-base': true,
          'text-red-500': !form.group_id,
        }"
        >所属部署</label
      >
      <select
        name="group_id"
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.group_id"
      >
        <option :value="0">未選択</option>
        <option v-for="group in props.groups" :key="group.id" :value="group.id">
          {{ group.name }}
        </option>
      </select>
    </div>
    <div
      class="sm:col-span-2"
      v-if="form.group_id == 3 || form.group_id == 4"
    >
      <label
        :class="{
          'mb-2 inline-block text-sm text-gray-800 sm:text-base': true,
          'text-red-500': form.group_id == 3 || form.group_id == 4,
        }"
        >製造工程</label
      >
      <p class="text-sm mb-4 text-gray-500">
        製造部に所属しない場合は、選択する必要はありません。
      </p>
      <select
        name="process_id"
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.process_id"
      >
        <option :value="0">未選択</option>
        <option
          v-for="process in props.processes"
          :value="process.id"
          :key="process.id"
        >
          {{ process.name }}
        </option>
      </select>
    </div>

    <div class="sm:col-span-2">
      <label
        :class="{
          'mb-2 inline-block text-sm text-gray-800 sm:text-base': true,
          'text-red-500': !form.position_id,
        }"
        >役職</label
      >
      <select
        name="position_id"
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.position_id"
      >
        <option :value="0">未選択</option>
        <option
          v-for="position in props.positions"
          :key="position.id"
          :value="position.id"
        >
          {{ position.name }}
        </option>
      </select>
    </div>

    <hr class="sm:col-span-2 my-8" />

    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >FAX振り分けフォルダ名</label
      >
      <p class="text-sm mb-4 text-gray-500">
        振り分ける必要がない場合、記載する必要はありません。
      </p>
      <input
        class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        type="text"
        v-model="form.fax_folder_name"
      />
    </div>

    <hr class="sm:col-span-2 my-8" />

    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >管理者フラグ</label
      >
      <br />
      <input
        type="checkbox"
        class="h-4 w-4 rounded border bg-gray-50 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.is_admin"
      />
    </div>
    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >非常勤・派遣フラグ</label
      >
      <br />
      <input
        type="checkbox"
        class="h-4 w-4 rounded border bg-gray-50 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.dispatch_flg"
      />
    </div>
    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >パート社員フラグ</label
      >
      <br />
      <input
        type="checkbox"
        class="h-4 w-4 rounded border bg-gray-50 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.part_flg"
      />
    </div>

    <div class="sm:col-span-2">
      <label class="mb-2 inline-block text-sm text-gray-800 sm:text-base"
        >弁当注文グループ参加フラグ</label
      >
      <br />
      <p class="text-sm mb-4 text-gray-500">
        頻繁に弁当を注文する場合は、チェックを入れてください。
      </p>
      <input
        type="checkbox"
        class="h-4 w-4 rounded border bg-gray-50 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring"
        v-model="form.always_order_flg"
      />
    </div>

    <div class="flex items-center justify-between sm:col-span-2 mt-8">
      <button
        type="button"
        @click="createUser"
        class="inline-block rounded-lg bg-indigo-500 px-8 py-3 text-center text-sm font-semibold text-white outline-none ring-indigo-300 transition duration-100 hover:bg-indigo-600 focus-visible:ring active:bg-indigo-700 md:text-base"
      >
        新規登録
      </button>

      <span class="text-sm text-gray-500">必須</span>
    </div>
  </form>
</template>
